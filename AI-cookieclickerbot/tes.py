    ...
    while not stop_event.is_set():
        ...
        try:
            if selenium_queue.qsize() < 20:
                ...
                # FIXED: Click fortune cookie icons in news ticker
                if time.time() - last_fortune_check > 1.0:
                    selenium_queue.put((lambda: driver.execute_script("""
                        try {
                            // Look for clickable elements in the news ticker
                            var fortuneLink = document.querySelector('#commentsText a');
                            if (fortuneLink) {
                                // Check if it's actually clickable and visible
                                var style = window.getComputedStyle(fortuneLink);
                                if (style.cursor === 'pointer' || fortuneLink.onclick) {
                                    fortuneLink.click();
                                    console.log('🥠 Clicked fortune cookie!');
                                }
                            }
                        } catch(e) {
                            console.log('Fortune click error:', e);
                        }
                    """), None))
                    last_fortune_check = time.time()
        except Exception:
...
try {
    ...
    for (var i = 0; i < M.goodsById.length; i++) {
        ...
        if (Array.isArray(g.vals) && g.vals.length > 0) {
            for (var j = 0; j < g.vals.length; j++) {
                ...
                if (typeof v === 'number' && !isNaN(v) && v > 0) {
                    if (allPrices.indexOf(v) === -1) allPrices.push(v);
                }
            }
        }
        
        if (cur > 0 && allPrices.indexOf(cur) === -1) allPrices.push(cur);
        if (allPrices.length > 500) allPrices = allPrices.slice(-500);
        ...
        var mean = validCount > 0 ? sum / validCount : cur;
        ...
        if (g.stock > 0 && mean > 0 && cur > mean * 1.12) {
            ...
            try { 
                ...
                trades.push('📉 SELL '+sellAmt+'x '+stockName+' @ $'+cur.toFixed(2)+' (mean: $'+mean.toFixed(2)+', +'+((cur/mean-1)*100).toFixed(1)+'%)');
            } catch(e){}
        }
        
        // BUY - FIXED: Correct cost calculation
        if (g.stock < maxStock && mean > 0 && cur < mean * 0.88) {
            var cookies = Game.cookies || 0;
            // Each stock unit costs: price * 1 (not * 100)
            var costPerUnit = cur;
            var maxAfford = Math.floor(cookies / costPerUnit);
            
            if (maxAfford >= 1) {
                var spaceAvailable = maxStock - g.stock;
                var buyAmt = Math.min(maxAfford, spaceAvailable, 100);
                
                var totalCost = buyAmt * costPerUnit;
                
                // Double-check we can actually afford it
                if (totalCost <= cookies) {
                    try {
                        M.buyGood(g.id, buyAmt);
                        trades.push('📈 BUY '+buyAmt+'x '+stockName+' @ $'+cur.toFixed(2)+' (cost: $'+totalCost.toFixed(2)+', mean: $'+mean.toFixed(2)+', -'+((1-cur/mean)*100).toFixed(1)+'%)');
                    } catch(e){
                        trades.push('❌ Failed to BUY '+buyAmt+'x '+stockName+' (error: '+e+')');
                    }
                }
            }
        }
